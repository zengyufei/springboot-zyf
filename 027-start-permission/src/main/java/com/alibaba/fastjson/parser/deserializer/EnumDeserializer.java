    ...
    public <T> T deserialze(DefaultJSONParser parser, Type type, Object fieldName) {
        try {
            ...
            if (token == JSONToken.LITERAL_INT) {
                ...
                for (Enum ordinalEnum : this.ordinalEnums) {
                    BaseEnum be = (BaseEnum) ordinalEnum;
                    Object indexObj = ReflectUtil.getFieldValue(be, "index");
                    if (Convert.toInt(indexObj, 0) == intValue) {
                        return (T) ordinalEnum;
                    }
                }
