...
public class MybatisPlusConfigiration {
     ...
     */
    @Bean
    @Profile({"dev", "test", "local-test"})// 设置 dev test 环境开启
    public PerformanceInterceptor performanceInterceptor() {
        PerformanceInterceptor performanceInterceptor = new PerformanceInterceptor();
        //performanceInterceptor.setFormat(true); // SQL是否格式化 默认false
        performanceInterceptor.setMaxTime(10000); // maxTime 指的是 sql 最大执行时长
    ...
    public MybatisMapperRefresh getMybatisMapperRefresh() throws IOException {
        ...
        Resource[] operations = resolver.getResources(this.mapperLocations);
