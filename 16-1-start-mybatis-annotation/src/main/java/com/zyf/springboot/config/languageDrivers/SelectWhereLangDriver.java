...
public class SelectWhereLangDriver extends XMLLanguageDriver implements LanguageDriver {

    private final Pattern inPattern = Pattern.compile("\\(#\\{(\\w+)\\}\\)");
    private final Pattern columnPattern = Pattern.compile("@(\\w+)@");
    ...
    public SqlSource createSqlSource(Configuration configuration, String script, Class<?> parameterType) {
        ...
        if (matcher.find()) {
            ...
            for (Field field : getFields(parameterType)) {
                ...
                if (field.isAnnotationPresent(Pass.class)
                        || field.isAnnotationPresent(PassWhere.class)) {
                    continue;
                }
                ...
                if (field.isAnnotationPresent(PassStart.class)) {
        ...
        Matcher columnFind = columnPattern.matcher(script);
        if (columnFind.find()) {
            script = columnFind.replaceAll("\\<include refid=\"$1\"\\/\\>");
        }
