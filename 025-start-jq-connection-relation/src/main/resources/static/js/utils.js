        ...
        $(array).each(function(){
            if(serializeObj[this.name]){
                if($.isArray(serializeObj[this.name])){
                    serializeObj[this.name].push(!isNaN(this.value) ? +this.value : this.value);
                }else{
                    serializeObj[this.name]=[serializeObj[this.name],!isNaN(this.value) ? +this.value : this.value];
                }
            }else{
                serializeObj[this.name]=!isNaN(this.value) ? +this.value : this.value;
            }
        });
        return serializeObj;
    };
    ...
    if(pageTotalPages <= 1) {
        ...
        for (var i=1; i<=pageTotalPages; i++) {
            if(pageCurrent === i) {
                ...
                pager.append('<input type="button" style="margin: 0 5px;" value="' + i + '" onclick="' +method(i)+ '"')
            }
        }
